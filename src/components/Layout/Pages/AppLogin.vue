    ...
    <template #sidebar>
      <div class="login-wrapper">
        <div class="login-form">
          ...
          <svws-ui-text-input placeholder="Server Adresse" />
          <svws-ui-text-input placeholder="Datenbank" />
          <svws-ui-button type="secondary">Verbinden</svws-ui-button>
          <svws-ui-text-input placeholder="Benutzername" />
          <svws-ui-text-input type="password" placeholder="Passwort" />
          <svws-ui-button>Anmelden</svws-ui-button>
        </div>
        <div class="login-footer">
          <img class="login-footer-logo" src="/img/NRW_MSB_RGB.jpg" />
          <p class="login-footer-hint">
            <span class="login-footer-hint-label">Hinweis:</span> Zur besseren
            Lesbarkeit werden bei SVWS-NRW ohne Diskriminierungsabsichten
            Begriffe wie Schüler, Lehrer, Erzieher usw. verwendet, womit
            Personen aller Geschlechtsidentitäten gemeint sind.
          </p>
...
<style>
.login-wrapper {
  @apply w-screen h-full px-16 flex flex-col justify-between;
  max-width: 581px;
}

.login-form {
  @apply space-y-4 py-48;
}
...
.login-footer {
  @apply space-y-5 flex-shrink-0 py-8;
}
...
.login-footer-link {
  @apply inline-block text-caption underline;
}

.login-background {
  background-image: url('/img/depositphotos_207646682-stock-photo-interior-empty-modern-classroom-school.jpg');
